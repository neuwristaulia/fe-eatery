    ...
    <div className="p-8 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button variant={filter === 'all' ? 'primary' : 'outline'} onClick={() => setFilter('all')} className="rounded-full">All</Button>
          <Button variant={filter === 'dine-in' ? 'primary' : 'outline'} onClick={() => setFilter('dine-in')} className="rounded-full">Dine In</Button>
          <Button variant={filter === 'takeaway' ? 'primary' : 'outline'} onClick={() => setFilter('takeaway')} className="rounded-full">Takeaway</Button>
        </div>
      </div>
                ...
                filteredOrders.map((order) => (
                  <tr key={order.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-muted-foreground">{order.type} {order.tableNumber ? `(${order.tableNumber})` : ''}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 flex justify-end gap-2">
                      {order.status === 'created' && (
                        <>
                          <Button size="sm" className="bg-green-600 hover:bg-green-700 text-white" onClick={() => confirmOrder(order.id)}>
                            <Check className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Confirm</span>
                          </Button>
                          <Button size="sm" variant="outline" className="text-red-500 hover:bg-red-500/10 hover:text-red-600 px-2" onClick={() => cancelOrder(order.id)}>
                            <X className="w-4 h-4" />
                          </Button>
                        </>
                      )}
                      ...
                      {order.status === 'confirmed' && (
                        <Button size="sm" disabled variant="secondary">
                          <Clock className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Waiting</span>
                        </Button>
                      )}
                      
                      {order.status === 'preparing' && (
                        <Button size="sm" disabled variant="secondary">
                          <ChefHat className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Cooking</span>
                        </Button>
                      )}
                      
                      {order.status === 'ready' && (
                        <Button size="sm" className="bg-primary hover:bg-primary/90 text-white" onClick={() => serveOrder(order.id)}>
                          <Utensils className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Serve</span>
                        </Button>
                      )}
                      
                      {order.status === 'served' && (
                        <Button size="sm" disabled className="bg-gray-100 text-gray-500 hover:bg-gray-100">
                          <CheckCircle className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Served</span>
                        </Button>
                      )}
                    </td>
                  </tr>
