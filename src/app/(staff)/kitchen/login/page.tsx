...
export default function KitchenLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!staffId.toUpperCase().startsWith("CHEF")) {
      setError("Akses ditolak. Silakan gunakan Kitchen ID yang valid.");
      return;
    }
    ...
    if (success) {
      ...
      setError("Kitchen ID atau Password salah.");
    }
  };
        ...
        <Card className="border-none shadow-2xl rounded-2xl overflow-hidden bg-card/80 backdrop-blur-md">
          <CardContent className="p-8">
            <div className="text-center mb-8">
              ...
              <h1 className="text-3xl font-serif font-bold text-foreground">Kitchen Portal</h1>
              <p className="text-muted-foreground mt-2">Masuk ke sistem Display Dapur (KDS).</p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-foreground">Kitchen ID</label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                  <input
                    type="text"
                    required
                    placeholder="Contoh: CHEF-01"
                    value={staffId}
            ...
            <div className="mt-8 text-center text-sm text-muted-foreground bg-muted/50 p-4 rounded-xl">
              ...
              <p>ID: <strong className="text-foreground">CHEF-01</strong> | Pass: <strong className="text-foreground">staff123</strong></p>
            </div>
          </CardContent>
        </Card>
      </motion.div>
