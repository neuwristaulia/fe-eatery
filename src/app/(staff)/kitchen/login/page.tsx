...
export default function KitchenLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const success = await login(userId, password, "kitchen");
    setIsLoading(false);

    if (success) {
      router.push("/kitchen/dashboard");
    } else {
      setError("User ID tidak valid atau bukan akun Kitchen.");
    }
  };
          ...
          <CardContent className="p-8">
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground mt-2">
                Masuk ke sistem Display Dapur (KDS).
              </p>
            </div>
            ...
            <div className="mt-8 text-center text-sm text-muted-foreground bg-muted/50 p-4 rounded-xl">
              ...
              <p>
                Nama: <strong className="text-foreground">kitchen01</strong> |
                Pass: <strong className="text-foreground">staff123</strong>
              </p>
            </div>
          </CardContent>
        </Card>
